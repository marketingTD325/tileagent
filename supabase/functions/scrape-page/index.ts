...
serve(async (req) => {
  ...
  try {
    ...
    // Extract title and meta description from HTML if available
    ...
    const result = {
      ...
      metadata: {
        title: titleMatch ? titleMatch[1].trim() : scrapedData.metadata?.title || '',
        description: metaDescMatch ? metaDescMatch[1].trim() : scrapedData.metadata?.description || '',
